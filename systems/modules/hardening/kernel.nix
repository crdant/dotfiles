...
let 
  ...
  bootConfig = lib.optionalAttrs supportsBoot {
    boot = {
      kernel.sysctl = {
        # Network hardening
  ...
  supportsDarwinKernel = builtins.hasAttr "defaults" options.environment;
  darwinKernelConfig = lib.optionalAttrs supportsDarwinKernel {
    environment.etc = {
      "sysctl.conf" = {
        text = ''
